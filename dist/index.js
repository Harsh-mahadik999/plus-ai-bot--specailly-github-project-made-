...
function makeDecision(intentScore, securityRisk) {
    if (securityRisk === 'CRITICAL')
        return 'BLOCK';
    if (securityRisk === 'HIGH' && intentScore < 70)
        return 'BLOCK';
    if (intentScore < 50)
        return 'BLOCK';
    if (securityRisk === 'MEDIUM' || intentScore < 80)
        return 'WARN';
    return 'APPROVE';
}
// ─── AI Analysis ──────────────────────────────────────────────────────────────
...
app.post('/webhook', async (req, res) => {
    ...
    try {
        ...
        await updateCheckRun(octokit, owner, repoName, checkRunId, decision, `Intent Alignment: ${aiResult.intentScore}% | Security: ${aiResult.securityRisk}`, aiResult.reasoning);
        ...
        if (checkRunId && token) {
            try {
                const octokit = new Octokit({ auth: token });
                await updateCheckRun(octokit, owner, repoName, checkRunId, 'WARN', 'Pulse encountered an error during analysis', err.message);
            }
            catch (e2) {
